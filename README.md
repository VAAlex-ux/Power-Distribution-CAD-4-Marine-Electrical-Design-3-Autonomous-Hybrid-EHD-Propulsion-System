...
The BMS tracks a sudden negative power derivative from the solar array (\(\frac{dI_{\text{solar}}}{dt} < \text{Threshold}_{\text{drop}}\)) due to cloud cover. The EHD sail is fully deactivated. The BMS commands a deployment system to drop the water turbine into the flow or activates the regeneration circuit on a hull-integrated unit. The ship's forward kinetic momentum (\(M_{\text{ship}} \cdot v\))
